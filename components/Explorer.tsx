...
import { MessageCircle, Heart, Calendar, ExternalLink, Filter, Loader2, Info, ThumbsDown, Edit3, LayoutGrid, List, ChevronRight, Home, Search, Star, StarOff, PanelLeftClose, PanelLeftOpen, Code2, Globe, Database, Cloud, Blocks, Smartphone, Wrench, Sparkles, Hash, Layers } from 'lucide-react';
...
const getTopicIcon = (id: string, size = 16) => {
  switch (id) {
    case 'javascript': return <Code2 size={size} />;
    case 'webdev': return <Globe size={size} />;
    case 'backend': return <Database size={size} />;
    case 'devops': return <Cloud size={size} />;
    case 'blockchain': return <Blocks size={size} />;
...
const Explorer: React.FC = () => {
  ...
  const initialParentTag = TOPICS.find(t => t.id === initialTag || t.sub.includes(initialTag))?.id || null;
  ...
  const [selectedParentTag, setSelectedParentTag] = useState<string | null>(initialParentTag);
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => {
    return (localStorage.getItem('explorer_view_mode') as 'grid' | 'list') || 'grid';
  });
  const [sidebarCollapsed, setSidebarCollapsed] = useState<'collapsed' | 'expanded'>(() => {
    return (localStorage.getItem('explorer_sidebar_mode') as 'collapsed' | 'expanded') || 'expanded';
  });
  ...
  const toggleFavorite = (e: React.MouseEvent, tagStr: string) => {
    ...
    setFavoriteTags(prev => {
        const newFavs = prev.includes(tagStr) ? prev.filter(t => t !== tagStr) : [...prev, tagStr];
  ...
  const filteredTopics = TOPICS.map(t => {
     ...
     const matchesParent = t.label.toLowerCase().includes(lowerSearch) || t.id.toLowerCase().includes(lowerSearch);
     const matchingSub = t.sub.filter(s => s.toLowerCase().includes(lowerSearch));
  ...
  useEffect(() => {
    const freshTag = searchParams.get('tag') || '';
    setTag(freshTag);
    setSelectedParentTag(TOPICS.find(t => t.id === freshTag || t.sub.includes(freshTag))?.id || null);
  }, [searchParams]);
  ...
  useEffect(() => {
    const init = async () => {
      const info = await getTribeInfo(community);
      setTribeInfo(info);
      fetchPosts(true);
    };
    init();
  ...
  const fetchPosts = async (reset: boolean, silent: boolean = false) => {
    ...
    const data = await getHivePosts(community, sort, 20, startAuthor, startPermlink, tag);
    
    if (reset) {
      setPosts(data);
    } else {
      const newPosts = data.filter(p => !posts.find(existing => existing.permlink === p.permlink));
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('byte-data-updated', handleByteUpdate);
  }, [sort, community, tag]);
  ...
  const getCalculatedReward = (post: HivePost) => {
    try {
      // 1. Check if post is paid out
      if (post.cashout_time) {
          const cashoutTime = new Date(post.cashout_time + (post.cashout_time.endsWith('Z') ? '' : 'Z')).getTime();
          const now = Date.now();
          if (cashoutTime < now) {
               // Post paid out logic
          }
      ...
      // 2. Fallback to simple pending_token from API if present and valid
      let pendingTokenVal = 0;
      if (post.pending_token != null && post.precision != null) {
        pendingTokenVal = post.pending_token / Math.pow(10, post.precision);
      }

      // 3. Dynamic Calculation using rshares (preferred for accuracy)
      if (tribeInfo && post.vote_rshares != null) {
          ...
          const precision = tribeInfo.precision != null ? tribeInfo.precision : 2;
      ...
      // 4. Return pending token if calculation failed but we have data
      if (pendingTokenVal > 0) {
        return pendingTokenVal.toFixed(tribeInfo?.precision || 2);
      }
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-8">
      ...
      <nav className="flex text-sm text-slate-400 font-medium whitespace-nowrap overflow-x-auto pb-2 scrollbar-none items-center gap-2">
         <Link to="/discovery" className="hover:text-white flex items-center gap-1.5 transition-colors">
            <Home size={14} /> Descobrir
         </Link>
         <ChevronRight size={14} className="opacity-50 shrink-0" />
         <Link to="/explorer" onClick={(e) => { if(!tag) e.preventDefault(); setSearchParams({}) }} className={`transition-colors ${!tag ? 'text-white' : 'hover:text-white'}`}>
            Explorer
         </Link>
         {tag && (
            <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center bg-card p-6 rounded-2xl border border-slate-700/50 shadow-lg relative overflow-hidden">
        ...
        <div className="w-full sm:w-auto mb-4 sm:mb-0 relative z-10">
          <h2 className="text-2xl lg:text-3xl font-extrabold text-white flex items-center gap-2">
            <span className="text-cent">#{community}</span> Community
          </h2>
          <p className="text-slate-400 text-sm mt-2 flex items-center gap-2 font-medium">
            ...
            {tribeInfo && tribeInfo.reward_pool && (
               <span className="bg-slate-800 text-xs px-2 py-0.5 rounded text-cent border border-cent/20 font-mono">
                 Pool: {parseFloat(tribeInfo.reward_pool).toFixed(0)} {community}
               </span>
            )}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto relative z-10">
          ...
          <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-700 w-full sm:w-auto overflow-x-auto shadow-inner">
            {(['created', 'trending', 'hot'] as const).map((s) => (
              ...
              >
                {s === 'created' ? 'Novos' : s === 'hot' ? 'Em Alta' : 'Populares'}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 relative">
        {/* Sidebar */}
        <div className={`w-full shrink-0 transition-all duration-300 ${sidebarCollapsed === 'expanded' ? 'lg:w-64' : 'lg:w-12 hidden lg:block'}`}>
          <div className="bg-card p-5 rounded-2xl border border-slate-700/50 shadow-lg sticky top-24">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-700/50">
               ...
               <button 
                 onClick={() => setSidebarCollapsed(prev => prev === 'expanded' ? 'collapsed' : 'expanded')}
                 className="p-1.5 rounded-md hover:bg-slate-800 text-slate-400 hover:text-white transition-colors"
                 title={sidebarCollapsed === 'expanded' ? 'Recolher Menu' : 'Expandir Menu'}
               >
                 {sidebarCollapsed === 'expanded' ? <PanelLeftClose size={18} /> : <PanelLeftOpen size={18} />}
               </button>
            </div>
              ...
              <>
                ...
                <div className="space-y-4">
                  ...
                  {favoriteTags.length > 0 && !categorySearch && (
                    <div>
                       <div className="text-xs uppercase tracking-wider text-slate-500 font-bold mb-2 px-1">Favoritos</div>
                       <div className="space-y-1">
                         {favoriteTags.map(favTag => {
                            const parentTopic = TOPICS.find(t => t.id === favTag || t.sub.includes(favTag));
                            return (
                              <div key={`fav-${favTag}`} className="relative group">
                                 <button
                                 ...
                                 >
                                    <span className="opacity-50">{parentTopic ? getTopicIcon(parentTopic.id, 14) : <Hash size={14} />}</span>
                                    <span>{favTag}</span>
                                 </button>
                                 <button
                                 ...
                                 >
                                    <StarOff size={14} className="text-slate-400 hover:text-red-400" />
                                 </button>
                              </div>
                            );
                         })}
                  ...
                  <div>
                    <div className="text-xs uppercase tracking-wider text-slate-500 font-bold mb-2 px-1">Todas Categorias</div>
                    <div className="space-y-1 max-h-[50vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                      {filteredTopics.map(t => (
                        <div key={t.id} className="space-y-1">
                          <div className="relative group">
                              <button 
                                onClick={() => {
                                   if (selectedParentTag === t.id && !t.expandAlways) {
                                      setSearchParams({});
                                   } else {
                              ...
                              >
                                <div className="flex items-center gap-2 truncate">
                                   <span className={selectedParentTag === t.id ? 'text-cent' : 'text-slate-500'}>
                              ...
                              <button
                                 ...
                                 title={favoriteTags.includes(t.id) ? "Remover favorito" : "Favoritar categoria"}
                              ...
                              >
                                 <Star size={14} className={favoriteTags.includes(t.id) ? 'fill-cent' : ''} />
                              </button>
                          </div>
                            ...
                            <div className="pl-3 py-1 space-y-1 border-l-2 border-slate-800 ml-3">
                              {t.sub.map(sub => (
                                <div key={sub} className="relative group">
                                    <button 
                                      onClick={() => setSearchParams({ tag: sub })}
                                    ...
                                    <button
                                       ...
                                       title={favoriteTags.includes(sub) ? "Remover favorito" : "Favoritar tag"}
                                    ...
                                    >
                                       <Star size={12} className={favoriteTags.includes(sub) ? 'fill-cent' : ''} />
                                    </button>
                                </div>
                              ))}
                            </div>
                      ...
                      {filteredTopics.length === 0 && (
                         <div className="text-center text-slate-500 text-sm py-4">Nenhuma categoria encontrada</div>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="flex-1 min-w-0 transition-all duration-300">
          {loading ? (
            <div className={viewMode === 'grid' ? `grid grid-cols-1 md:grid-cols-2 ${sidebarCollapsed === 'collapsed' ? 'lg:grid-cols-3 xl:grid-cols-4' : 'lg:grid-cols-2 xl:grid-cols-3'} gap-6` : "flex flex-col gap-4"}>
               {[1, 2, 3, 4, 5, 6].map(i => (
                 <div key={i} className={`bg-card animate-pulse border border-slate-700/50 ${viewMode === 'grid' ? "rounded-xl h-80" : "rounded-xl h-36 flex flex-col justify-center"}`}>
                    {viewMode === 'grid' && <div className="h-40 bg-slate-700/50 rounded-t-xl"></div>}
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-card rounded-2xl border border-slate-700/50">
               ...
               <p className="text-slate-400 max-w-md mb-6">
                 Ainda não há postagens aqui. Seja o primeiro a criar um conteúdo incrível{tag ? <span> com a tag <span className="font-mono text-cent bg-cent/10 px-1.5 py-0.5 rounded">#{tag}</span></span> : ''}!
               </p>
               <Link to="/create-post" className="flex items-center gap-2 bg-cent text-slate-900 px-6 py-2.5 rounded-xl font-bold hover:bg-cent/90 transition-all shadow-lg shadow-cent/20">
                 <Edit3 size={18} />
                 Criar Postagem
               </Link>
            </div>
          ) : (
            <div className={viewMode === 'grid' ? `grid grid-cols-1 md:grid-cols-2 ${sidebarCollapsed === 'collapsed' ? 'lg:grid-cols-3 xl:grid-cols-4' : 'lg:grid-cols-2 xl:grid-cols-3'} gap-6` : "flex flex-col gap-5"}>
              {posts.map((post) => {
                const thumbnail = extractImage(post);
                const reward = getCalculatedReward(post);
                const isPaid = post.cashout_time && new Date(post.cashout_time + 'Z').getTime() < Date.now();
                ...
                const mainTag = postTags.find(tag => allOurTags.has(tag)) || postTags.find(tag => tag !== community && tag !== 'hive-192096') || postTags[0];
                
                return (
                  <div key={`${post.author}-${post.permlink}`} className={`group bg-card rounded-2xl overflow-hidden border border-slate-700/50 hover:border-slate-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-cent/5 flex ${viewMode === 'list' ? 'flex-col sm:flex-row items-stretch' : 'flex-col h-full'}`}>
                    {viewMode === 'grid' && (
                      <div className="relative h-48 bg-slate-800 overflow-hidden shrink-0">
                        {thumbnail ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-slate-800 text-slate-600">
                            <span className="font-mono text-xs">Sem Imagem</span>
                          </div>
                        )}
                        <div className={`absolute top-3 right-3 ${isPaid ? 'bg-slate-700/90 text-slate-300' : 'bg-cent/90 text-slate-900'} backdrop-blur-sm font-bold px-2.5 py-1 rounded-md text-xs flex items-center gap-1 font-mono shadow-lg`}>
                          {reward} {community}
                        </div>
                        {mainTag && (
                    ...
                    <div className={`p-5 flex flex-col flex-1 ${viewMode === 'list' ? 'justify-between' : ''}`}>
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <Link to={`/profile/${post.author}`} className="text-xs font-bold text-slate-300 hover:text-cent cursor-pointer truncate">
                          @{post.author}
                        </Link>
                      ...
                      >
                        <h3 className={`font-bold text-white leading-tight mb-2 ${viewMode === 'list' ? 'text-xl' : 'text-lg line-clamp-2'}`}>
                          {post.title}
                        </h3>
                        {excerpt && (
                          <p className={`text-slate-400 text-sm leading-relaxed font-medium ${viewMode === 'grid' ? 'line-clamp-3' : 'line-clamp-2'} mb-2`}>
                            {excerpt}
                          </p>
                        )}
                      </Link>
                      
                      <div className={`flex flex-wrap items-center justify-between gap-4 mt-4 pt-4 border-t border-slate-700/50 text-slate-400 text-xs`}>
                         <div className="flex gap-4 items-center">
                           <div className="flex items-center gap-1.5 transition-colors">
                             <button 
                               ...
                               className={`${
                                 userHasVoted ? 'text-green-400' : 'hover:text-green-300 text-slate-400'
                               }`} 
                               title={userHasVoted ? "Você já votou" : "Votar 100%"}
                             >
                               {isVotingThis ? (
                                 <Loader2 size={16} className="animate-spin" />
                               ) : (
                                 <Heart size={16} className={userHasVoted ? "fill-green-400" : ""} />
                               )}
                             </button>
                             <button 
                           ...
                           {down > 0 && (
                             <span className="flex items-center gap-1.5 text-red-400/90 hover:text-red-300 transition-colors" title="Downvotes">
                               <ThumbsDown size={14} className="fill-red-400/20" /> <span className="font-medium text-sm">{down}</span>
                             </span>
                           )}
                           ...
                           >
                             <MessageCircle size={16} /> <span className="font-medium text-sm">{post.children}</span>
                           </Link>
                         </div>
                         
                         <div className="flex items-center gap-3">
                           {viewMode === 'list' && (
                             <div className={`px-2.5 py-1 rounded font-bold flex items-center gap-1 font-mono text-xs ${isPaid ? 'text-slate-400 bg-slate-800' : 'text-cent bg-cent/10'}`}>
                               {reward} {community}
                             </div>
                           )}
                           <a 
