    ...
    const voters = post.active_votes
      .map(vote => {
        ...
        if (vRshares > 0 && totalRshares > 0 && postCalculatedReward > 0) {
            // Simplified proportion based on rshares
      ...
      .filter(v => v.reward >= 1) // "quem tiver menos que 1, nem precisa mostrar"
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {votersList.length === 0 ? (
            <div className="p-8 text-center text-slate-500">
               Nenhum voto com valor maior ou igual a 1 {community}.
            </div>
          ) : (
