...
import { getPostContent, getTribeInfo, getScotPost, getPostReplies } from '../services/hiveEngineService';
...
const CommentItem: React.FC<{ comment: HivePost, tribeInfo: TribeInfo | null, readerSettings: any }> = ({ comment, tribeInfo, readerSettings }) => {
  ...
  const getCalculatedCentReward = (p: HivePost) => {
    try {
      const precision = p.precision ?? tribeInfo?.precision ?? 2;
      if (tribeInfo && p.vote_rshares != null) {
          ...
          if (pendingRshares > 0 && rewardPool > 0 && !isNaN(rshares)) {
              const estimate = ((Math.pow(rshares, exponent)) * rewardPool) / pendingRshares;
              if (!isNaN(estimate)) {
  ...
  const handleVote = async () => {
    ...
    const alreadyVoted = localComment.active_votes?.some(v => v.voter === user);
    if (alreadyVoted) {
    ...
    setIsVoting(true);
    try {
      const result = await vote(localComment.author, localComment.permlink, 10000);
      if (result.success) {
         setLocalComment(prev => ({
             ...prev,
             net_votes: (prev.net_votes || 0) + 1,
             active_votes: [...(prev.active_votes || []), { voter: user, weight: 10000, percent: 10000, rshares: 0 }]
         }));
      } else {
  ...
  const handleReply = async () => {
    ...
    setIsReplying(true);
    try {
      const result = await postComment(localComment.author, localComment.permlink, '', replyText, ['cent'], declinePayout);
      if (result.success) {
  ...
  const userHasVoted = user && localComment.active_votes?.some(v => v.voter === user);
  ...
  return (
    <div className="border-b border-slate-800 py-6 last:border-0 relative group">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
           ...
           <div className="flex items-center gap-2">
              <Link to={`/profile/${localComment.author}`} className="font-bold text-slate-300 text-sm hover:text-cent transition-colors">@{localComment.author}</Link>
              <span className="text-xs text-slate-500">• {timeAgo(localComment.created)}</span>
           </div>
        </div>
        <div className="flex items-center gap-3">
          <button 
      ...
      <div className="flex items-center gap-6 mt-4 pl-11">
         ...
         >
           {isVoting ? <Loader2 size={14} className="animate-spin" /> : <Heart size={14} className={userHasVoted ? "fill-current" : ""} />}
           {netVotes}
         </button>
         ...
         <div className="mt-4 pl-11 animate-fade-in">
           ...
           <div className="flex justify-between items-center mt-2">
             <label className="flex items-center gap-2 text-xs text-slate-400 cursor-pointer hover:text-white transition-colors">
               <input type="checkbox" checked={declinePayout} onChange={e => setDeclinePayout(e.target.checked)} className="accent-cent bg-slate-800" /> Decline Payout
             </label>
             <div className="flex justify-end gap-2">
               <button onClick={() => setShowReplyBox(false)} className="px-3 py-1.5 text-xs text-slate-400 hover:text-white">Cancelar</button>
               <button
                 onClick={handleReply}
               ...
               >
                 {isReplying ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Responder
               </button>
             </div>
           </div>
         </div>
...
const SinglePost: React.FC = () => {
  const { community } = useCommunity();
  const { author: rawAuthor, permlink } = useParams<{ author: string; permlink: string }>();
  const author = rawAuthor?.startsWith('@') ? rawAuthor.substring(1) : rawAuthor;
  ...
  const loadPostData = async (a: string, p: string) => {
    ...
    if (hiveData) {
      ...
      const { vote_rshares, pending_payout_value, total_payout_value, active_votes: hiveVotes, ...safeHiveData } = hiveData;
      ...
      if ((!tokenVoteRshares || Number(tokenVoteRshares) === 0) && scotData?.active_votes) {
         const calculatedRshares = scotData.active_votes.reduce((acc: number, v: any) => acc + Number(v.rshares || 0), 0);
      ...
      setDebugData({
        ...
        sample_vote: scotData?.active_votes && scotData.active_votes.length > 0 ? scotData.active_votes[0] : null
      });
  ...
  const getCalculatedCentReward = (post: HivePost) => {
    try {
      const precision = post.precision ?? tribeInfo?.precision ?? 2;
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      if (result.success) {
         ...
         alert("Voto computado! Atualize a página em alguns instantes para ver o novo valor.");
      } else {
        alert("Erro ao votar: " + result.msg);
      }
    } catch(e) {
  ...
  const handleReply = async () => {
    ...
    setIsReplying(true);
    try {
      const result = await postComment(post.author, post.permlink, '', replyText, ['cent'], declinePayout);
      if (result.success) {
        alert("Comentário publicado! Atualize a página em alguns instantes para ver seu comentário.");
        setReplyText('');
  ...
  if (!post) {
    return (
      <div className="text-center mt-12">
        ...
        <button onClick={() => navigate('/explorer')} className="mt-4 text-cent underline">Voltar para o Explorer</button>
      </div>
    );
  }

  const { up, down } = post.active_votes ? post.active_votes.reduce((acc, v) => {
    const pct = v.percent !== undefined ? v.percent : 1; 
    if (pct > 0) acc.up++;
    else if (pct < 0) acc.down++;
    return acc;
  }, { up: 0, down: 0 }) : { up: post.net_votes, down: 0 };

  const userHasVoted = user && post.active_votes?.some(v => v.voter === user);
  const rewardValue = getCalculatedCentReward(post);
  const isPaid = post.cashout_time && new Date(post.cashout_time + (post.cashout_time.endsWith('Z') ? '' : 'Z')).getTime() < Date.now();

  const content = (
    <div className={`max-w-4xl mx-auto pb-12 animate-fade-in relative ${isModal ? 'pt-8 px-4' : ''}`}>
      {!isModal && (
        <button 
          onClick={handleBack} 
      ...
      <div className="bg-card rounded-2xl border border-slate-700/50 overflow-hidden shadow-2xl relative">
        {isModal && (
          <button 
            onClick={handleBack}
        ...
        <div className="p-8 border-b border-slate-700/50 pt-12 md:pt-8">
           <h1 className="text-3xl md:text-4xl font-bold text-white mb-6 leading-tight pr-8">
             {post.title}
           </h1>
           ...
           <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <Link to={`/profile/${post.author}`} className="font-bold text-slate-200 hover:text-cent transition-colors block">@{post.author}</Link>
              ...
              <div className="flex flex-wrap gap-3 items-center">
                 <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-mono font-bold text-sm shadow-lg ${isPaid ? 'bg-slate-700 text-slate-300' : 'bg-cent/10 text-cent border border-cent/20'}`}>
                    <Coins size={16} />
        ...
        <div className="p-8 bg-slate-900/50">
           <article 
             className={`prose ${readerSettings.fontSize} ${readerSettings.fontFamily} max-w-none text-slate-300
             ...
             prose-code:text-slate-100 prose-code:bg-slate-800 prose-pre:bg-slate-800 prose-pre:text-slate-300
             prose-img:rounded-xl prose-img:shadow-lg prose-img:mx-auto transition-all`}
            ...
            <div className="mt-8 pt-6 border-t border-slate-700/50 flex flex-wrap items-center justify-between gap-4">
              <div className="flex flex-wrap gap-2">
                 {(() => {
                    ...
                    const uniqueTags = Array.from(new Set([post.category, ...tags, community])).slice(0, 8);
                    
                    return uniqueTags.map(tag => (
                       <span key={tag} className="text-xs font-medium text-slate-400 bg-slate-800 px-2.5 py-1 rounded-md border border-slate-700">
                         #{tag}
                       </span>
                    ));
                 })()}
              ...
              >
                 {copiedAuthorperm ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
        ...
        <div className="p-6 bg-slate-800 border-t border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
           <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-start">
              <div className="flex items-center gap-4">
                <div className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all font-bold ${
                  userHasVoted 
                  ...
                  <button 
                    ...
                    title="Votar (100%)"
                  >
                     {isVoting ? <Loader2 size={24} className="animate-spin" /> : <Heart size={24} className={`transition-transform group-active:scale-90 ${userHasVoted ? "fill-current" : ""}`} />}
                  </button>
                  <button 
                ...
                <div 
                  ...
                  title="Downvotes"
                >
                   <ThumbsDown size={24} className={down > 0 ? "fill-current" : ""} />
        ...
        {showSettings && (
          <div className="bg-slate-900 p-6 border-t border-slate-700 font-sans">
             <div className="flex justify-between items-center mb-6">
               <h3 className="text-lg font-bold text-white flex items-center gap-2"><Settings size={18}/> Configurações de Leitura</h3>
               <button onClick={() => setShowSettings(false)} className="text-slate-400 hover:text-white"><X size={20}/></button>
             </div>
             
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                   <label className="block text-sm font-medium text-slate-400 mb-2">Tamanho da Fonte</label>
                   <div className="flex bg-slate-800 rounded-lg p-1">
                      {['prose-sm', 'prose-base', 'prose-lg', 'prose-xl'].map(size => (
                         <button
                           key={size}
                         ...
                         >
                           {size === 'prose-sm' ? 'P' : size === 'prose-base' ? 'M' : size === 'prose-lg' ? 'G' : 'GG'}
                         </button>
                      ))}
                   </div>
                </div>
                <div>
                   <label className="block text-sm font-medium text-slate-400 mb-2">Estilo da Fonte</label>
                   <div className="flex bg-slate-800 rounded-lg p-1">
                      {[
                         ...
                         <button
                           key={font.id}
                           onClick={() => setReaderSettings(s => ({ ...s, fontFamily: font.id }))}
        ...
        {showDebug && (
          <div className="bg-black/50 p-6 border-t border-slate-700 font-mono text-xs text-slate-400">
             <div className="flex justify-between items-center mb-4">
               <h3 className="text-sm font-bold text-cent flex items-center gap-2"><Bug size={14}/> Debug de Cálculo</h3>
               <button onClick={() => setShowDebug(false)} className="hover:text-white"><X size={16}/></button>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                   <p className="text-white font-bold border-b border-slate-700 pb-1">Variáveis do Post</p>
                   <div className="flex justify-between"><span>RShares Usado:</span> <span className="text-yellow-400">{debugData.final_used_rshares?.toLocaleString() || 0}</span></div>
                   <div className="flex justify-between"><span>Origem RShares:</span> <span>{debugData.rshares_source}</span></div>
                   <div className="flex justify-between"><span>Scot Raw RShares:</span> <span>{debugData.scot_vote_rshares_raw || 'null'}</span></div>
                   <div className="flex justify-between"><span>Api Pending Token:</span> <span className="text-red-400">{debugData.scot_pending_token || 0} (Ignorado)</span></div>
                   <div className="flex justify-between"><span>Active Votes (Scot):</span> <span>{debugData.active_votes_count}</span></div>
                   <div className="flex justify-between"><span>Exemplo Voto:</span> <span className="text-xs break-all">{JSON.stringify(debugData.sample_vote || {})}</span></div>
                </div>
                <div className="space-y-2">
                   <p className="text-white font-bold border-b border-slate-700 pb-1">Variáveis da Tribe</p>
                   <div className="flex justify-between"><span>Reward Pool:</span> <span className="text-green-400">{tribeInfo?.reward_pool}</span></div>
                   <div className="flex justify-between"><span>Pending RShares:</span> <span className="text-blue-400">{tribeInfo?.pending_rshares}</span></div>
                   <div className="flex justify-between"><span>Author Curve Exp:</span> <span>{tribeInfo?.author_curve_exponent}</span></div>
                </div>
             </div>
          </div>
        )}
        ...
        <div className="p-8 bg-slate-900 border-t border-slate-800">
          <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
            ...
            Comentários {post.children > 0 && <span className="text-slate-500 text-sm">({post.children})</span>}
          </h3>
          ...
          {user ? (
                 ...
                 <div className="flex-1">
                   ...
                   <div className="flex justify-between items-center mt-2">
                     ...
                     <div className="flex justify-end mt-2">
                       ...
                       >
                         {isReplying ? <Loader2 size={18} className="animate-spin" /> : <MessageCircle size={18} />}
                         Comentar
                       </button>
                     </div>
                   </div>
            ...
            <div className="mb-8 p-4 bg-yellow-400/10 border border-yellow-400/20 rounded-xl flex items-center justify-between text-yellow-500">
              <span className="text-sm font-medium">Faça login para comentar neste post.</span>
            </div>
          )}
          ...
          {loadingComments ? (
            ...
            <div className="space-y-4">
              {comments.map((comment) => (
                <CommentItem key={`${comment.author}-${comment.permlink}`} comment={comment} tribeInfo={tribeInfo} readerSettings={readerSettings} />
              ))}
            </div>
          ) : (
            <div className="text-center py-8 text-slate-500 bg-slate-800/50 rounded-lg border border-slate-700/30">
              <p>Nenhum comentário ainda. Seja o primeiro a comentar no Hive!</p>
            </div>
          )}
        </div>
      </div>
  ...
  if (isModal) {
    return (
      <div 
        className="fixed inset-0 z-50 flex justify-center items-start overflow-y-auto bg-black/80 backdrop-blur-sm sm:px-4 sm:py-8"
        onMouseDown={(e) => {
